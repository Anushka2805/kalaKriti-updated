...
export default function FairsShowcase() {
  ...
  useEffect(() => {
    speak(
      "Yeh Indian artisan fairs ka page hai. Yahan desh ke mashhoor mela aur craft festivals dikhaye gaye hain. Aap bol sakte ho next, repeat, ya kisi fair ka naam."
    );
    listen(handleFairVoice);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const handleFairVoice = (text: string) => {
    text = text.toLowerCase();

    // Next fair
    if (text.includes("next") || text.includes("aage")) {
      const nextIndex = (currentIndex + 1) % fairs.length;
      setCurrentIndex(nextIndex);
      const f = fairs[nextIndex];
      speak(
        `${f.name}, location ${f.location}. ${f.description}`
      );
      listen(handleFairVoice);
      return;
    }
    ...
    // Repeat current
    if (text.includes("repeat") || text.includes("dobara")) {
      const f = fairs[currentIndex];
      speak(
        `${f.name}, location ${f.location}. ${f.description}`
      );
      listen(handleFairVoice);
    ...
    // Specific fair by name
    const foundIndex = fairs.findIndex((f) =>
      text.includes(f.name.toLowerCase())
    );

    if (foundIndex !== -1) {
      setCurrentIndex(foundIndex);
      const f = fairs[foundIndex];
      speak(
        `${f.name}, location ${f.location}. ${f.description}`
      );
      listen(handleFairVoice);
        ...
        {fairs.map((fair, index) => (
          ...
          >
            ...
            <div className="p-6">
              ...
              <p className="text-sm text-[#a06b5b] mt-1">
                {fair.location}
              </p>
